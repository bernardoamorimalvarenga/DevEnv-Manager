...
import os
import json
import click  # ← ADICIONAR ESTA LINHA
...
"""
                ...
                subprocess.run(
                    ["git", "commit", "-m", "Initial DevEnv Manager sync setup"], 
                    cwd=self.sync_dir, 
                    check=True, 
                    capture_output=True
                )
            ...
            console.print(f"[green]✓ Git sync setup successfully with {remote_url}[/green]")
            return True
    ...
    def push_environments(self, environment_names: Optional[List[str]] = None) -> bool:
        ...
        try:
            ...
            console.print(f"[blue]📤 Pushing {len(environment_names)} environments...[/blue]")
            ...
            if result.stdout.strip():
                ...
                console.print(f"[green]✓ Successfully pushed {synced_count} environments[/green]")
            else:
    ...
    def pull_environments(self) -> bool:
        ...
        try:
            console.print("[blue]📥 Pulling environments from remote...[/blue]")
            ...
            if not env_dir.exists():
                console.print("[yellow]No environments found in remote repository[/yellow]")
                return True
            ...
            for env_file in env_dir.glob("*.json"):
                try:
                    ...
                    if existing_data:
                        ...
                        remote_time = data.get('metadata', {}).get('created_at', '')
                        local_time = existing_data.get('metadata', {}).get('created_at', '')
                        
                        if remote_time <= local_time:
                            console.print(f"[dim]Skipping {env_name} (local version is newer)[/dim]")
                            continue
                        
                        console.print(f"[yellow]Environment '{env_name}' already exists locally[/yellow]")
                        if not click.confirm(f"Overwrite local version of '{env_name}'?"):
                            continue
                    ...
                    console.print(f"[red]✗ Failed to import {env_file.name}: {e}[/red]")
            
            console.print(f"[green]✓ Successfully imported {imported_count} environments[/green]")
            return True
    ...
    def sync_status(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                "enabled": True,
                "remote_url": self.remote_url or config.get('sync.remote_url'),
                "branch": "main",
    ...
    def _check_sync_setup(self) -> bool:
        ...
        if not config.get('sync.enabled'):
            console.print("[red]✗ Git sync not setup. Run 'devenv sync setup <repo-url>' first[/red]")
            return False
        
        if not self.sync_dir.exists() or not (self.sync_dir / ".git").exists():
            console.print("[red]✗ Git sync directory corrupted. Run setup again[/red]")
            return False

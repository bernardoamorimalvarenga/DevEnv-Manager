...
def init():
    ...
    console.print(Panel.fit(
        "[bold green]🚀 DevEnv Manager initialized successfully![/bold green]\n"
...
def capture(name):
    ...
    console.print(f"[bold blue]📸 Capturing environment: {name}[/bold blue]")
...
def list():
    ...
    if not snapshots:
        console.print("[yellow]📋 No environments found. Use 'devenv capture <name>' to create one.[/yellow]")
        return
...
def restore(name, dry_run, force):
    ...
    if 'packages' in data:
        ...
        if not dry_run and not force:
            console.print(f"\n[yellow]⚠️  This will install {total_packages} packages and may modify your system.[/yellow]")
...
def delete(name, force):
    ...
    # Confirmation
    if not force:
        console.print(f"[yellow]⚠️  This will permanently delete environment '{name}'[/yellow]")
...
def import_env(input_file, name):
    ...
    try:
        ...
        if storage.save_snapshot(env_name, data):
            console.print(f"[green]✓ Environment imported as '{env_name}'![/green]")
        else:
...
def diff(env1, env2):
    ...
    if 'vscode_extensions' in data1 and 'vscode_extensions' in data2:
        _compare_extensions(data1['vscode_extensions'], data2['vscode_extensions'], env1, env2)
...
def push(env):
    ...
    if env and len(env) > 0:
        ...
        console.print(f"[blue]📤 Pushing specific environments: {', '.join(env_list)}[/blue]")
    else:
        env_list = None
    ...
    if result:
        if env_list:
            console.print(f"[green]✓ Successfully pushed {len(env_list)} specific environments[/green]")
        else:
